...
/**
 * Calculates difference between two dates and returns that difference in days (hours, minutes and seconds)
 *
 * Return fromat is [number of days]d [hours]:[minutes]:[seconds]
 *
 * @author Stevan Knezevic - stevan.knezevic@mikroe.com
 * @since  10.05.2019
 * @param  string     $start_date Valid date
 * @param  string     $end_date   Valid date
 * @param  string     $format     format of passed dates
 * @return string or false on failure
 */
	public function dateDiffInDays($start_date, $end_date, $format = 'Y-m-d H:i:s') {
		...
		$dateInterval = $start_date->diff($end_date);

		if (!$dateInterval) {
			return false;
		}

		$date_string = $dateInterval->days . 'd ' .
			sprintf('%02d', $dateInterval->h) . ':' .
			sprintf('%02d', $dateInterval->i) . ':' .
			sprintf('%02d', $dateInterval->s)
		;
