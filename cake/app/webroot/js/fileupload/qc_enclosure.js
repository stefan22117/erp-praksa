$("#container").ready(function() {	 			
	$(function () {
		'use strict';
		$('#QcEnclosureAttachment').fileupload({
			url: '/QcEnclosures/upload/',
			dataType: 'json',
			done: function (e, data) {			
				//console.log(data.result);
				if(data.result.error){
				    $("#QcEnclosureFileLink").val("");
					$("#upload_info").html('<span style="color:red;">Greška: '+data.result.error+'</span>');
				}else{					
					$("#QcEnclosureFileLink").val(data.result.filepath);
					$("#upload_info").html('<span style="color:blue;">Fajl je uspešno zakačen!</span>');
					//console.log($("#QcEnclosureFileLink").val());
				}
