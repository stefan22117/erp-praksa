...
/**
 * QueuedTask::runWorker()
 * Start worker in background
 * @author marko.jovanovic (marko@mikroe.com)
 * @return false if BackgroundProcess class not available
 */
	public function runWorker() {
		$debug = Configure::read('debug');
		if(empty($debug)){
	        if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
	        	$cmd = 'php '.ROOT.DS.'app'.DS.'Console'.DS.'cake.php Queue.Queue runworker';
	        }else{
	        	$file_owner = get_current_user();
	        	$cmd = PHP_BINDIR.'/php /home/'.$file_owner.'/www/cake/app/Console/cake.php Queue.Queue runworker';                 
	        }

	        //Init background process
	        App::import('Vendor', 'BackgroundProcess', array('file' => 'Cocur'.DS.'BackgroundProcess'.DS.'BackgroundProcess.php'));
